...
int main(int argc, char *argv[])
{
    ...
    if (hayFilas && hayColumnas && hayArchivo && hayNProcesos && hayPorcentaje)
    {
       matriz = reservarMatriz(matriz, filas, columnas);
        ...
        cargaDatos(arch, matriz, filas, columnas, &valido);
        fclose(arch);
        //Reviso si los valores de filas y columnas son menores o iguales a la del archivo
        if(!valido)
        {
        ...
        //inicio a tomar el tiempo que se demoran los procesos en realizar el analisis
        ...
        //ahora termino de tomar el tiempo para posteriormente imprimirlo
        segundos = detener_tiempo(&inicio);
    ...
    printf("\nEl tiempo que demoraron los procesos en analizar la matriz fue %.7f segundos\n", segundos);
    return 0;
}
